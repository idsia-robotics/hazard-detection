...
from utils.rnvp_auc_computation_utils import \
    per_label_rnvp_metrics, rnvp_auc_computation_and_logging
from utils.variables_util import \
    combined_labels_to_names
from models.real_nvp.real_nvp_model_functions import \
    build_network, model_tester, set_oe_parameters, epoch_loop_oe_with_oe_rnvp
from RealNVP.real_nvp_dataset import OEEmbeddingsDataset, OEEmbeddingsOutliersSet, OEEmbeddingsTestSet
...
def main():
    ...
    model_key = f"RNVP_OE_E_{embedding_size}_T_{coupling_topology}_N_{n_layers}_{time_string}"
    save_folder = f"{root_path}/data/rnvp_oe/saves/{model_key}"
    ...
    test_set = OEEmbeddingsTestSet(
        file_path=test_set_path
    )

    len_train_loader = len(train_loader)
    final_size_oe = outlier_batch_size * len_train_loader

    outliers_set = OEEmbeddingsOutliersSet(
        file_path=outliers_set_path,
        required_dataset_size=final_size_oe,
    )
    ...
    # TRAIN
    metrics, best_model_path = epoch_loop_oe_with_oe_rnvp(
        device,
        epochs,
        model,
        optimizer,
